...
RECT noteRect14(int r,int s){int top=(int)editorArea11.top,gridTop=top+42,gridBottom=(int)editorArea11.bottom-12,labelW=86;int rh=std::max(12,(gridBottom-gridTop)/std::max(1,NOTES));int cw=std::max(8,((int)editorArea11.right-labelW-14)/STEPS);int y=gridTop+r*rh;return{(LONG)(labelW+s*cw),(LONG)y,(LONG)(labelW+(s+1)*cw-1),(LONG)std::min(gridBottom,y+rh-1)};}
bool noteCell14(POINT p,int&row,int&step){if(!editorOpen||laneSelected<0||laneSelected>=(int)lanes.size()||lanes[laneSelected].type!=LaneType::Instrument)return false;for(int r=0;r<NOTES;r++)for(int s=0;s<STEPS;s++)if(inside(noteRect14(r,s),p)){row=r;step=s;return true;}return false;}
...
void drawV14(HDC d){
 ...
 for(int r=0;r<NOTES;r++){RECT q=noteRect14(r,0);if(q.bottom<=q.top)continue;RECT lab{0,q.top,85,q.bottom};fill(d,lab,((base-r)%12==0)?RGB(35,38,45):RGB(23,25,30));std::wstring nm=noteName14(base-r);label(d,nm.c_str(),22,(int)q.top+std::max(2,(int)(q.bottom-q.top)/2-5),8,((base-r)%12==0)?RGB(240,120,88):RGB(157,162,173),FW_BOLD);}
 for(int s=0;s<STEPS;s+=4){RECT first=noteRect14(0,s),last=noteRect14(NOTES-1,s);if(first.right<=first.left)continue;line(d,first.left,first.top,first.left,last.bottom,RGB(82,87,98),2);std::wstring beat=std::to_wstring(s/4+1);label(d,beat.c_str(),first.left+5,first.top-16,7,RGB(145,150,161),FW_BOLD);}
