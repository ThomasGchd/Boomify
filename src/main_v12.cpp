...
RECT master12{},nav12{},navThumb12{},zoomMinus12{},zoomPlus12{},barsPlus12{};
...
void syncSelection12(int lane,int bar,bool add,bool extend){
    ...
    if(extend && anchorLane12>=0){
        clearMulti(); int l0=std::min(anchorLane12,lane),l1=std::max(anchorLane12,lane),b0=std::min(anchorBar12,bar),b1=std::max(anchorBar12,bar);
        for(int l=l0;l<=l1;l++)for(int b=b0;b<=b1;b++)addMulti(l,b);
    }else{
        if(!add)clearMulti(); bool found=false;for(auto&q:multiSelection)if(q.track==lane&&q.bar==bar){found=true;break;}
        if(add&&found)multiSelection.erase(std::remove_if(multiSelection.begin(),multiSelection.end(),[&](const BlockRef&q){return q.track==lane&&q.bar==bar;}),multiSelection.end());else addMulti(lane,bar);
...
void setNav12(int x){int maxScroll=std::max(0,activeBars-visibleBars);if(maxScroll<=0){scrollBar=0;return;}int travel=std::max(1,(int)(nav12.right-nav12.left)-(int)(navThumb12.right-navThumb12.left));int pos=std::clamp(x-(int)nav12.left-(int)(navThumb12.right-navThumb12.left)/2,0,travel);scrollBar=std::clamp((int)((long long)pos*maxScroll/travel),0,maxScroll);InvalidateRect(win,nullptr,FALSE);}
...
void overlay12(HDC d){
    ...
    if(bar>=scrollBar&&bar<scrollBar+visibleBars){int v=bar-scrollBar,w=std::max(1,(int)(timeline10.right-timeline10.left));int x=(int)timeline10.left+(int)((long long)v*w/visibleBars);line(d,x,(int)timeline10.top,x,(int)timeline10.bottom,RGB(255,104,75),3);RECT tag{(LONG)x,(LONG)timeline10.top,(LONG)std::min((int)timeline10.right,x+38),(LONG)((int)timeline10.top+20)};fill(d,tag,RGB(235,103,74));std::wstring n=std::to_wstring(bar+1);label(d,n.c_str(),x+8,(int)timeline10.top+5,7,RGB(255,255,255),FW_BOLD);}
    master12={(LONG)std::max(920,(int)c.right-330),24,(LONG)std::max(1040,(int)c.right-190),34};label(d,L"MASTER",master12.left-58,24,7,RGB(150,154,162),FW_BOLD);fill(d,master12,RGB(35,38,45));RECT mf=master12;mf.right=mf.left+(mf.right-mf.left)*master/100;fill(d,mf,RGB(235,103,74));std::wstring mv=std::to_wstring(master)+L"%";label(d,mv.c_str(),master12.right+7,22,7,RGB(190,193,200),FW_BOLD);
    int nx=310,nr=(int)timeline10.right-20;nav12={(LONG)nx,88,(LONG)nr,100};fill(d,nav12,RGB(35,38,45));int total=std::max(1,activeBars),trackW=std::max(1,nr-nx),thumbW=std::clamp((int)((long long)trackW*visibleBars/total),34,trackW);int maxScroll=std::max(0,activeBars-visibleBars),travel=std::max(0,trackW-thumbW),tx=nx+(maxScroll?scrollBar*travel/maxScroll:0);navThumb12={(LONG)tx,86,(LONG)(tx+thumbW),102};fill(d,navThumb12,RGB(101,107,119));
    zoomMinus12={(LONG)(nr-112),76,(LONG)(nr-84),106};zoomPlus12={(LONG)(nr-80),76,(LONG)(nr-52),106};barsPlus12={(LONG)(nr-48),76,(LONG)nr,106};fill(d,zoomMinus12,RGB(45,48,56));fill(d,zoomPlus12,RGB(45,48,56));fill(d,barsPlus12,RGB(45,48,56));label(d,L"-",zoomMinus12.left+10,85,9,RGB(220,222,226),FW_BOLD);label(d,L"+",zoomPlus12.left+9,84,9,RGB(220,222,226),FW_BOLD);label(d,L"+8",barsPlus12.left+14,84,8,RGB(220,222,226),FW_BOLD);
...
LRESULT CALLBACK proc_v12(HWND h,UINT m,WPARAM wp,LPARAM lp){
    ...
    if(m==WM_LBUTTONDOWN){
        if(inside(master12,p)){dragMode12=1;setMaster12(p.x);SetCapture(h);return 0;}
        if(inside(zoomMinus12,p)){zoom(-1);return 0;}if(inside(zoomPlus12,p)){zoom(1);return 0;}if(inside(barsPlus12,p)){addBars();layout11(h);return 0;}
        if(inside(nav12,p)||inside(navThumb12,p)){dragMode12=2;setNav12(p.x);SetCapture(h);return 0;}
        ...
        if(inside(timeline10,p)){int lane=-1,bar=-1;bool onCell=hitTimeline11(p,lane,bar);setPlayhead12(p);if(onCell){bool ctrl=(GetKeyState(VK_CONTROL)&0x8000)!=0,shift=(GetKeyState(VK_SHIFT)&0x8000)!=0;syncSelection12(lane,bar,ctrl,shift);if(!ctrl&&!shift){range12=true;rangeStart12=rangeNow12=p;SetCapture(h);}}InvalidateRect(h,nullptr,FALSE);return 0;}
    }
