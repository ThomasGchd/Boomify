...
void header16(HDC d,RECT c){
    ...
    // Existing V14 save/open hitboxes are preserved but visually modernized.
    save14={438,16,506,50};open14={514,16,588,50};pill16(d,save14,L"SAVE",PURPLE16);pill16(d,open14,L"OPEN",CYAN16);
    // transport: reuse real hit rectangles so buttons remain functional
...
void timeline16(HDC d){
    RECT p=panels15.timeline;fill(d,p,BG16);RECT inner{p.left+7,p.top+8,p.right-7,p.bottom-8};rr16(d,inner,18,SURF16,RGB(44,46,65),1);int left=inner.left+8,right=inner.right-8,top=inner.top+PANEL_HEADER15;
    ...
    for(int l=0;l<(int)lanes.size();++l){RECT rr=row15(l);rr.left=left;rr.right=right;rr.top+=4;rr.bottom-=4;if(rr.bottom<=rr.top)continue;rr16(d,rr,10,(l&1)?RGB(20,22,34):RGB(23,25,38));for(int v=0;v<visibleBars;v++){int b=scrollBar+v;if(b>=MAX_BARS)break;int x0=left+(int)((long long)v*tw/std::max(1,visibleBars)),x1=left+(int)((long long)(v+1)*tw/std::max(1,visibleBars));if(b<activeBars&&lanes[l].type!=LaneType::Audio&&laneUsed11(l,b)){COLORREF cc=colors15[l]?colors15[l]:defaultColor15(l);RECT q{x0+4,rr.top+3,x1-4,rr.bottom-3};if(selectedCell15(l,b))glow16(d,q,cc,3);rr16(d,q,10,cc,selectedCell15(l,b)?RGB(255,255,255):cc,selectedCell15(l,b)?2:0);}}
        if(lanes[l].type==LaneType::Audio){RECT ar=audioRect15(l);if(ar.right>ar.left){ar.left=std::max(ar.left,left);ar.right=std::min(ar.right,right);COLORREF cc=colors15[l]?colors15[l]:defaultColor15(l);rr16(d,ar,10,cc);drawWave15(d,{ar.left+4,ar.top+3,ar.right-4,ar.bottom-3},samples15[l],cc);}}
    }
    int bar=playing?playBar12():cursorBar;if(bar>=scrollBar&&bar<scrollBar+visibleBars){int x=left+(int)((long long)(bar-scrollBar)*tw/std::max(1,visibleBars));line(d,x,inner.top+30,x,inner.bottom-7,ORANGE16,2);RECT cap{x-5,inner.top+27,x+6,inner.top+36};rr16(d,cap,5,ORANGE16);}
}
void inspector16(HDC d){
    RECT p=panels15.inspector;fill(d,p,BG16);RECT in{p.left+8,p.top+8,p.right-10,p.bottom-8};rr16(d,in,18,SURF16,RGB(48,50,70),1);int x=in.left+14,w=std::max(120,(int)(in.right-in.left)-28),y=in.top+12;
    text16(d,L"INSPECTOR",{x,y,x+w,y+22},8,PURPLE16,FW_BOLD);y+=26;if(laneSelected>=0&&laneSelected<(int)lanes.size()){COLORREF cc=colors15[laneSelected]?colors15[laneSelected]:defaultColor15(laneSelected);text16(d,lanes[laneSelected].name,{x,y,x+w,y+32},14,RGB(245,245,250),FW_HEAVY);fill(d,{x,y+34,x+50,y+37},cc);y+=48;}
    import15={x,(LONG)y,x+w,(LONG)(y+34)};pill16(d,import15,L"IMPORT WAV",CYAN16);y+=42;boom15={x,(LONG)y,x+w,(LONG)(y+34)};pill16(d,boom15,L"+ BOOMBOX",ORANGE16);y+=44;master15={x,(LONG)y,x+w,(LONG)(y+38)};pill16(d,master15,L"MASTER  "+std::to_wstring(master)+L"%",PURPLE16,masterOn15);y+=50;
    addFx15={x,(LONG)y,x+w,(LONG)(y+36)};pill16(d,addFx15,L"+ ADD EFFECT",PINK16,true);y+=48;text16(d,L"FX CHAIN",{x,y,x+w,y+22},7,RGB(145,150,170),FW_BOLD);y+=25;fxMinus15.fill(RECT{});fxPlus15.fill(RECT{});
    if(laneSelected>=0&&laneSelected<(int)lanes.size()){auto&r=rack11[laneSelected];for(int i=0;i<(int)r.size()&&i<6&&y+40<in.bottom;i++){RECT fr{x,(LONG)y,x+w,(LONG)(y+38)};COLORREF cc=fxColor11(r[i].type);rr16(d,fr,12,CARD16,RGB(54,56,78),1);fill(d,{fr.left,fr.top+8,fr.left+4,fr.bottom-8},cc);text16(d,fxName11(r[i].type),{fr.left+12,fr.top,fr.right-92,fr.bottom},7,RGB(229,231,240),FW_BOLD);fxMinus15[i]={fr.right-84,fr.top+7,fr.right-60,fr.bottom-7};fxPlus15[i]={fr.right-26,fr.top+7,fr.right-2,fr.bottom-7};pill16(d,fxMinus15[i],L"-",PURPLE16);pill16(d,fxPlus15[i],L"+",PURPLE16);text16(d,std::to_wstring(r[i].amount),{fr.right-58,fr.top,fr.right-28,fr.bottom},7,CYAN16,FW_BOLD,DT_CENTER|DT_VCENTER|DT_SINGLELINE);y+=44;}}
}
void editorFrame16(HDC d,RECT c){
    ...
    // The real editor remains the functional owner. This pass adds modern framing only.
...
void paint16(HWND h,HDC out){
    sync16(h);RECT c{};GetClientRect(h,&c);HDC d=CreateCompatibleDC(out);HBITMAP bm=CreateCompatibleBitmap(out,std::max(1L,c.right),std::max(1L,c.bottom));auto ob=SelectObject(d,bm);fill(d,c,BG16);
    // Run legacy painters off-screen first only to keep their live hit geometry, then fully cover the modern workspace.
    draw11(d,c);drawV13Chrome(d);drawV14(d);
    header16(d,c);tracks16(d);timeline16(d);inspector16(d);editorFrame16(d,c);
    // redraw the functional lower editor content inside its own area after the modern frame
